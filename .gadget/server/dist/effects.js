...
function createGadgetRecord(apiIdentifier, data) {
    ...
    const baseRecord = new api_client_core_1.GadgetRecord({
...
async function save(record) {
    ...
    const api = (0, utils_1.assert)(context ? context.api : getRequestContext().api, "api client is missing from the current context");
    const model = getModelByTypename(record.__typename);
...
async function preventCrossShopDataAccess(params, record, options) {
    ...
    if ((0, utils_1.isObjectLike)(input)) {
...
async function globalShopifySync(params) {
    const context = maybeGetActionContextFromLocalStorage();
    const effectAPIs = (0, utils_1.assert)(context ? context.effectAPIs : getRequestContext().effectAPIs, "effect apis is missing from the current context");
    const api = (0, utils_1.assert)(context ? context.api : getRequestContext().api, "api client is missing from the current context");
    ...
    if (apiKeys && apiKeys.length > 0) {
        ...
        for (const result of results) {
            ...
            if ((0, utils_1.isEmpty)(result[accessTokenIdentifier]) || result.state?.created == "uninstalled") {
...
function getRequestContext() {
    return globals_1.Globals.requestContext.get("requestContext");
}
const LINK_PARAM = "_link";
function writableAttributes(model, record) {
    const fieldsByApiIdentifier = (0, utils_1.keyBy)(Object.values(model.fields), "apiIdentifier");
    return (0, utils_1.pickBy)(record, (v, k) => fieldsByApiIdentifier[k]?.internalWritable);
}
